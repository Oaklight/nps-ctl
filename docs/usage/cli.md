...
编辑已有隧道。定位参数（`--id`、`-r/--remark`、`-p/--port` + `-t/--type`）用于定位目标隧道；`--new-*` 参数指定修改内容。

**定位选项：**

| 选项              | 说明                                                       |
| ----------------- | ---------------------------------------------------------- |
| `--id`            | 隧道 ID（Edge 特定，需配合 `-e`）— 与 `--remark` 互斥     |
| `-r`, `--remark`  | 隧道备注名称（可跨 Edge 定位）— 与 `--id` 互斥            |
| `-p`, `--port`    | 服务端端口（配合 `-t/--type` 跨 Edge 定位）                |
| `-t`, `--type`    | 隧道类型（配合 `-p/--port` 跨 Edge 定位）                  |
| `-e`, `--edge`    | Edge 名称（`--id` 时必填，其他定位方式可选）               |
...
编辑已有域名映射。定位参数用于定位目标 host；`--new-*` 参数指定修改内容。

**定位选项：**

| 选项              | 说明                                                         |
| ----------------- | ------------------------------------------------------------ |
| `--id`            | Host ID（Edge 特定，需配合 `-e`）— 与 `--domain`、`--remark` 互斥 |
